...
- **24h-Testphase**: Free-User (mit `profiles.plan != 'paid'` und ohne laufende Testphase) können sich in `/einstellungen` per Button (`/api/start-trial.ts`) einmalig für 24 Stunden den bezahlten Plan freischalten (`profiles.trial_ends_at`). `getEffectivePlan()`/`isTrialActive()` in `src/lib/supabase.ts` berechnen den wirksamen Plan bei jedem Request dynamisch aus `plan` + `trial_ends_at` — kein Cronjob nötig, der Rückfall auf "Kostenlos" passiert automatisch, sobald `trial_ends_at` in der Vergangenheit liegt. Nach Ablauf ist ein erneuter Start wieder möglich. Die Restlaufzeit wird im (fixierten) Footer live heruntergezählt (`trial-countdown`-Script in `Footer.astro`)
